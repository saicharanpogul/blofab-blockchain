...
# Exception
export CORE_PEER_TLS_ROOTCERT_FILE_BLOFAB=/${PWD}/organizations/peerOrganizations/$ORG_NAME1.$DOMAIN_NAME1.com/peers/peer0.$ORG_NAME1.$DOMAIN_NAME1.com/tls/ca.crt
export CORE_PEER_TLS_ROOTCERT_FILE_NBTC=/${PWD}/organizations/peerOrganizations/$ORG_NAME2.$DOMAIN_NAME2.com/peers/peer0.$ORG_NAME2.$DOMAIN_NAME2.com/tls/ca.crt
...
installChaincode() {
    ...
    peer lifecycle chaincode install ../chaincode/${CHAINCODE_NAME}.tgz --peerAddresses localhost:$ORG_PORT --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE}
...
checkCommitReadiness() {
  ...
  peer lifecycle chaincode checkcommitreadiness -o localhost:9050 --channelID ${CHANNEL_NAME} --tls --cafile ${ORDERER_CA} --name ${CHAINCODE_NAME} --version ${CHAINCODE_VERSION} --sequence 1 --output json --init-required --signature-policy "AND ('${ORG_NAME1^}MSP.peer','${ORG_NAME2^}MSP.peer')"
...
commitChaincode() {
  ...
  peer lifecycle chaincode commit -o localhost:$PORT --ordererTLSHostnameOverride orderer$ORDERER_NUMBER.$ORG_NAME.$DOMAIN_NAME.com --tls ${CORE_PEER_TLS_ENABLED} --cafile ${ORDERER_CA} --channelID ${CHANNEL_NAME} --name ${CHAINCODE_NAME} --peerAddresses localhost:7051 --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE_BLOFAB} --peerAddresses localhost:9051 --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE_NBTC} --version ${CHAINCODE_VERSION} --sequence 1 --init-required
...
initChaincode() {
  ...
  peer chaincode invoke -o localhost:9050 --ordererTLSHostnameOverride orderer$ORDERER_NUMBER.$ORG_NAME.$DOMAIN_NAME.com --tls ${CORE_PEER_TLS_ENABLED} --cafile ${ORDERER_CA} -C ${CHANNEL_NAME} -n ${CHAINCODE_NAME} --peerAddresses localhost:7051 --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE_BLOFAB} --peerAddresses localhost:9051 --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE_NBTC} --isInit -c '{"Args":[]}'
