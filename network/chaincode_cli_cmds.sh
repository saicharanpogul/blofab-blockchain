...
commitChaincode () {
  ...
  docker exec cli_${ORG_NAME1} peer lifecycle chaincode commit \
...
initChaincode () {
  ...
  docker exec cli_${ORG_NAME1} peer chaincode invoke \
  -o orderer0.${ORG_NAME1}.${DOMAIN_NAME}.com:7050 \
  --ordererTLSHostnameOverride orderer0.${ORG_NAME1}.${DOMAIN_NAME}.com \
  --tls true \
