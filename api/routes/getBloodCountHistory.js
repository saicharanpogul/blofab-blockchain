...
router.get('/', async (req, res, next) => {
  try {
    ...
    // GetBloodCountHistory(ctx contractapi.TransactionContextInterface, bloodBankId string)
    const result = await contract.evaluateTransaction(
      'GetBloodCountHistory',
      req.body.bloodBankId
    )

    console.log('Query blood count history transaction has been evaluated.')
    res.status(201).json({
      status: "successful",
      message: "Query blood count history transaction has been evaluated.",
    ...
    console.log(`Failed to evaluate query blood count history transaction: ${error.message}`)
    res.status(400).json({
      status: "failed",
