...
router.put('/', async (req, res, next) => {
  try {
    ...
    // IncrementBloodCount(ctx contractapi.TransactionContextInterface, bloodBankId string, bloodType string) 
    ...
    console.log('Increment transaction has been submitted.')
    res.status(201).json({
      status: "successful",
      message: "Increment transaction has been submitted.",
    ...
    console.log(`Failed to evaluate increment transaction: ${error.message}`)
    res.status(400).json({
      status: "failed",
